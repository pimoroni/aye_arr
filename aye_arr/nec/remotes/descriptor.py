...
ButtonCallbacks = namedtuple("ButtonCallbacks", ("on_press", "on_short", "on_repeat", "on_release"))
...
class RemoteDescriptor:
    ...
    def bind(self, name, on_press, on_short=None, on_repeat=True, on_release=None):
        if name not in self.BUTTON_CODES:
            raise KeyError(f"Name '{name}' is not a bindable button of the '{self.NAME}' remote")

        self.bind_code(self.BUTTON_CODES[name], on_press, on_short, on_repeat, on_release)

    def bind_code(self, code, on_press, on_short=None, on_repeat=True, on_release=None):
